...
public class TestNoteService extends BaseTest {
  ...
  public void setUp() throws Exception {
    ...
    removeWikiPages(noteService, PortalConfig.PORTAL_TYPE, PORTAL_NAME);
  }
  ...
  public void testGetGroupPageById() throws WikiException, IllegalAccessException {
    Wiki wiki = getOrCreateWiki(wService, PortalConfig.GROUP_TYPE, USERS_GROUP);
    Identity root = ROOT_IDENTITY;

    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.GROUP_TYPE, USERS_GROUP, "Home"));

    Page notePage = noteService.createNote(wiki, "Home", new Page("testGetGroupPageById-101", "testGetGroupPageById-101"), root);

    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.GROUP_TYPE, USERS_GROUP, notePage.getName()));
  ...
  public void testCreatePageAndSubNote() throws WikiException, IllegalAccessException {
    ...
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, parentNotePage.getName(), root));
    Page childNotePage = noteService.createNote(wiki, parentNotePage.getName(), new Page("childPage_", "childPage_"), root);
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, childNotePage.getName(), root));
  }

  public void testGetBreadcumb() throws WikiException, IllegalAccessException {
    Identity root = ROOT_IDENTITY;
    Wiki portalWiki = getOrCreateWiki(wService, PortalConfig.PORTAL_TYPE, PORTAL_NAME);
    Page breadcrumb1NotPage = noteService.createNote(portalWiki, "Home", new Page("Breadcrumb1_", "Breadcrumb1_"), root);
    Page breadcrumb2NotPage = noteService.createNote(portalWiki,
                                                     breadcrumb1NotPage.getName(),
                                                     new Page("Breadcrumb2_", "Breadcrumb2_"),
                                                     root);
    Page breadcrumb3NotPage = noteService.createNote(portalWiki,
                                                     breadcrumb2NotPage.getName(),
                                                     new Page("Breadcrumb3_", "Breadcrumb3_"),
                                                     root);
    List<BreadcrumbData> breadCumbs = noteService.getBreadCrumb(PortalConfig.PORTAL_TYPE,
                                                                PORTAL_NAME,
                                                                breadcrumb3NotPage.getName(),
                                                                false);
  ...
  public void testGetBreadcumbWithLanguage() throws WikiException, IllegalAccessException {
    ...
    List<BreadcrumbData> breadCumbs = noteService.getBreadCrumb(PortalConfig.PORTAL_TYPE, PORTAL_NAME, note3.getName(), false);
    assertEquals(4, breadCumbs.size());
    assertEquals("Home", breadCumbs.get(0).getId());
    ...
    breadCumbs = noteService.getBreadCrumb(PortalConfig.PORTAL_TYPE, PORTAL_NAME, note3.getName(), "fr", root, false);
    assertEquals(4, breadCumbs.size());
    assertEquals("Home", breadCumbs.get(0).getId());
  ...
  public void testMoveNote() throws WikiException, IllegalAccessException {
    ...
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, oldParentNotePage.getName()));
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, childNotePage.getName()));
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, newsParentNotPage.getName()));
    ...
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, classicNotePage.getName(), root));
    ...
    Wiki demoWiki = getOrCreateWiki(wService, PortalConfig.USER_TYPE, "root");
    ...
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.USER_TYPE, "root", toMovedNotePage.getName()));
  ...
  public void testDeleteNote() throws WikiException, IllegalAccessException {
    ...
    assertTrue(noteService.deleteNote(PortalConfig.PORTAL_TYPE, PORTAL_NAME, notePage1.getName()));
    // wait(10) ;
    Page notePage2 = noteService.createNote(portalWiki, "Home", new Page("deletePage_", "deletePage_"), root);
    assertTrue(noteService.deleteNote(PortalConfig.PORTAL_TYPE, PORTAL_NAME, notePage2.getName()));
    assertNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, notePage2.getName()));
    assertFalse(noteService.deleteNote(PortalConfig.PORTAL_TYPE, PORTAL_NAME, "Home"));
  }
  ...
  public void testRenameNote() throws WikiException, IllegalAccessException {
    ...
    assertTrue(noteService.renameNote(PortalConfig.PORTAL_TYPE,
                                      PORTAL_NAME,
                                      currentNotePage.getName(),
                                      "renamedPage_",
                                      "renamedPage_"));
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, "renamedPage_"));
  }
  ...
  public void testRenamePageToExistingNote() throws WikiException, IllegalAccessException {
    ...
    try {
      noteService.renameNote(PortalConfig.PORTAL_TYPE,
                             PORTAL_NAME,
                             currentNotePage.getName(),
                             currentNotePage2.getName(),
                             "renamedPage2_");
    ...
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, currentNotePage.getName()));
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, currentNotePage2.getName()));
  }

  public void testHasPermisionOnSystemPage() throws WikiException, IllegalAccessException {
    NoteDataStorage storage = getContainer().getComponentInstanceOfType(NoteDataStorage.class);
    ...
    Identity adminAclIdentity = new Identity("admin",
                                             Arrays.asList(
                                                           new MembershipEntry(USERS_GROUP, "*"),
  ...
  public void testUpdateNote() throws WikiException, IllegalAccessException, Exception {
    Identity root = ROOT_IDENTITY;
    // Get Home
    getOrCreateWiki(wService, PortalConfig.PORTAL_TYPE, PORTAL_NAME).getWikiHome();

    // Create a wiki page for test
    Page page = new Page("new page_", "new page_");
    page.setContent("Page content");
    page = noteService.createNote(new Wiki(PortalConfig.PORTAL_TYPE, PORTAL_NAME), "Home", page, root);
    assertNotNull(page);
  ...
  public void testDraftPage() throws WikiException, IllegalAccessException {
    ...
    draftOfNewPage = noteService.createDraftForNewPage(new DraftPage(), now, getIdentityId(ROOT_IDENTITY.getUserId()));
    ...
    DraftPage updatedDraftOfNewPage = noteService.updateDraftForNewPage(draftOfNewPage, now, getIdentityId(ROOT_IDENTITY.getUserId()));
    ...
    DraftPage updatedDraftOfExistingPage = noteService.updateDraftForExistPage(draftOfExistingPage,
                                                                               targetPage,
                                                                               null,
                                                                               now,
                                                                               root.getUserId());
  ...
  public void testGEtNoteById() throws WikiException, IllegalAccessException {
    ...
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, note1.getName()));
  ...
  public void testExportNotes() throws Exception {
    ...
    note3 = noteService.updateNote(note3, PageUpdateType.EDIT_PAGE_CONTENT, ROOT_IDENTITY);

    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, note1.getName()));
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, note2.getName()));
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, note3.getName()));
  ...
  public void testImportNotes() throws Exception {
    Wiki portalWiki = getOrCreateWiki(wService, PortalConfig.PORTAL_TYPE, PORTAL_NAME);
    assertThrows(IllegalAccessException.class, () -> noteService.createNote(portalWiki, "Home", new Page("to_be_imported1", "to_be_imported1"), USER_IDENTITY));
    Page note1 = noteService.createNote(portalWiki, "Home", new Page("to_be_imported1", "to_be_imported1"), ROOT_IDENTITY);
    Page note2 = noteService.createNote(portalWiki, "Home", new Page("to_be_imported2", "to_be_imported2"), ROOT_IDENTITY);
    Page note3 = noteService.createNote(portalWiki, "Home", new Page("to_be_imported3", "to_be_imported3"), ROOT_IDENTITY);
    ...
    notesExportService.startExportNotes(200231, notes, true, ROOT_IDENTITY);
    boolean exportDone = false;
    while (!exportDone) {
      if (notesExportService.getStatus(200231).getStatus().equals("ZIP_CREATED")) {
        exportDone = true;
  ...
  public void testGetNotesOfWiki() throws WikiException, IllegalAccessException {
    Wiki portalWiki = getOrCreateWiki(wService, PortalConfig.PORTAL_TYPE, PORTAL_NAME);
    Page toBeImported1NotPage = noteService.createNote(portalWiki, "Home", new Page("to_be_imported1", "to_be_imported1"), ROOT_IDENTITY);
    Page toBeImported2NotPage = noteService.createNote(portalWiki, "Home", new Page("to_be_imported2", "to_be_imported2"), ROOT_IDENTITY);
    Page toBeImported3NotPage = noteService.createNote(portalWiki, "Home", new Page("to_be_imported3", "to_be_imported3"), ROOT_IDENTITY);

    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, toBeImported1NotPage.getName()));
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, toBeImported2NotPage.getName()));
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, toBeImported3NotPage.getName()));
  ...
  public void testDeleteNote1() throws Exception {
    Identity root = ROOT_IDENTITY;
    Wiki portalWiki = getOrCreateWiki(wService, PortalConfig.PORTAL_TYPE, PORTAL_NAME);
    Page notePage = noteService.createNote(portalWiki, "Home", new Page("note1", "note1"), root);
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, notePage.getName()));
    noteService.deleteNote(PortalConfig.PORTAL_TYPE, PORTAL_NAME, notePage.getName(), root);
    assertNull(noteService.getNoteOfNoteBookByName(PortalConfig.PORTAL_TYPE, PORTAL_NAME, notePage.getName()));
  }
  ...
  public void testDeleteVersionsByNoteIdAndLang() throws Exception {
    ...
    Wiki portalWiki = getOrCreateWiki(wService, PortalConfig.GROUP_TYPE, space.getGroupId());
    Page note1 = noteService.createNote(portalWiki, "Home", new Page("testPage1", "testPage1"), root);
    assertNotNull(noteService.getNoteOfNoteBookByName(PortalConfig.GROUP_TYPE, space.getGroupId(), note1.getName()));
  ...
  public void testGetChildrenNoteOf() throws WikiException, IllegalAccessException {
    Wiki portalWiki = getOrCreateWiki(wService, PortalConfig.PORTAL_TYPE, PORTAL_NAME);
    noteService.createNote(portalWiki, "Home", new Page("imported1", "imported1"), ROOT_IDENTITY);
    noteService.createNote(portalWiki, "Home", new Page("imported2", "imported2"), ROOT_IDENTITY);
  ...
  public void testGetPublishedVersionByPageIdAndLang() throws WikiException, IllegalAccessException {
    Identity root = ROOT_IDENTITY;
    Wiki portalWiki = getOrCreateWiki(wService, PortalConfig.PORTAL_TYPE, PORTAL_NAME);
    Page note = noteService.createNote(portalWiki, "Home", new Page("testNotee", "testNotee"), root);
    ...
    noteService.createVersionOfNote(note, root.getUserId());
  ...
  public void testRemoveOrphanDraftPagesByParentPage() throws Exception {
    startSessionAs("root");
    Wiki portalWiki = getOrCreateWiki(wService, PortalConfig.GROUP_TYPE, USERS_GROUP);
    ...
    draft = noteService.createDraftForNewPage(draft, new Date().getTime(), getIdentityId(ROOT_IDENTITY.getUserId()));
    assertNotNull(draft);
  ...
  private Page createTestNoteWithVersionLang(String name, String lang, Identity user) {
    ...
    Wiki spaceWiki = getOrCreateWiki(wService, PortalConfig.GROUP_TYPE, space.getGroupId());
    Page note = noteService.createNote(spaceWiki, "Home", new Page(name, name), ROOT_IDENTITY);
    note.setLang(lang);
  ...
  public void testSaveNoteFeaturedImage() throws Exception {
    Page note = createTestNoteWithVersionLang("testMetadata", "en", ROOT_IDENTITY);

    bindMockedUploadService();

    NotePageProperties notePageProperties = createNotePageProperties(Long.parseLong(note.getId()), "alt text", "summary test");
    NotePageProperties properties = noteService.saveNoteMetadata(notePageProperties, null, getIdentityId(ROOT_IDENTITY.getUserId()));
    assertEquals("summary test", properties.getSummary());

    notePageProperties.setSummary("version language summary");
    properties = noteService.saveNoteMetadata(notePageProperties, "en", getIdentityId(ROOT_IDENTITY.getUserId()));
    assertEquals("version language summary", properties.getSummary());
  }

  public void testRemoveNoteFeaturedImage() throws Exception {
    Page note = createTestNoteWithVersionLang("testMetadataRemove", "fr", ROOT_IDENTITY);

    this.bindMockedUploadService();

    NotePageProperties notePageProperties = createNotePageProperties(Long.parseLong(note.getId()), "alt text", "summary test");
    NotePageProperties properties = noteService.saveNoteMetadata(notePageProperties, null, getIdentityId(ROOT_IDENTITY.getUserId()));
    noteService.saveNoteMetadata(notePageProperties, "fr", getIdentityId(ROOT_IDENTITY.getUserId()));

    assertNotNull(properties.getFeaturedImage().getId());
    NoteFeaturedImage noteFeaturedImageInfo = noteService.getNoteFeaturedImageInfo(Long.parseLong(note.getId()),
                                                                                   null,
                                                                                   false,
                                                                                   null,
    ...
    assertNull(noteService.getNoteFeaturedImageInfo(Long.parseLong(note.getId()), null, false, null, getIdentityId(ROOT_IDENTITY.getUserId())));
    assertNull(noteService.getNoteFeaturedImageInfo(Long.parseLong(note.getId()), "fr", false, null, getIdentityId(ROOT_IDENTITY.getUserId())));
  }

  public void testGetNoteFeaturedImageInfo() throws Exception {
    Page note = createTestNoteWithVersionLang("testGetMetadataInfo", "ar", ROOT_IDENTITY);

    this.bindMockedUploadService();

    NotePageProperties notePageProperties = createNotePageProperties(Long.parseLong(note.getId()), "alt text", "summary Test");
    noteService.saveNoteMetadata(notePageProperties, null, getIdentityId(ROOT_IDENTITY.getUserId()));
    notePageProperties = createNotePageProperties(Long.parseLong(note.getId()), "alt text", "summary Test");
    noteService.saveNoteMetadata(notePageProperties, "ar", getIdentityId(ROOT_IDENTITY.getUserId()));
    NoteFeaturedImage featuredImage = noteService.getNoteFeaturedImageInfo(Long.parseLong(note.getId()),
                                                                           null,
                                                                           false,
                                                                           "150x150",
                                                                           getIdentityId(ROOT_IDENTITY.getUserId()));
  ...
  public void testCreatePageWithProperties() throws Exception {
    ...
    draftPage = noteService.createDraftForNewPage(draftPage, new Date().getTime(), getIdentityId(ROOT_IDENTITY.getUserId()));
    Wiki portalWiki = getOrCreateWiki(wService, PortalConfig.PORTAL_TYPE, PORTAL_NAME);
    ...
    assertThrows(IllegalAccessException.class, () -> noteService.createNote(portalWiki, "Home", page, USER_IDENTITY));

    Page note = noteService.createNote(portalWiki, "Home", page, ROOT_IDENTITY);
    assertNotNull(note);
    ...
    note = noteService.createNote(portalWiki, "Home", page, ROOT_IDENTITY);
    assertNotNull(note);
    assertNotNull(note.getProperties());
  }
  ...
  public void testGetVersionById() throws Exception {
    Wiki portalWiki = getOrCreateWiki(wService, PortalConfig.PORTAL_TYPE, PORTAL_NAME);
    Page note = noteService.createNote(portalWiki, "Home", new Page("testGetVersionById", "testGetVersionById"), ROOT_IDENTITY);
    note.setLang("en");
    note.setTitle("english title");
    note.setContent("english content");
    noteService.createVersionOfNote(note, USER_IDENTITY.getUserId());
  ...
  public void testFeaturedImageWhenRemoveDraftById() throws Exception {
    ...
    NoteFeaturedImage featuredImage =
                                    noteService.getNoteFeaturedImageInfo(Long.valueOf(draftPage.getId()), null, true, null, getIdentityId(ROOT_IDENTITY.getUserId()));
    assertNotNull(featuredImage);
    noteService.removeDraftById(draftPage.getId());
    assertNull(fileService.getFile(featuredImage.getId()));

    // Draft related to page
    // Once deleted we should be aware of illustration assigned to parent page
    // or not
    Page page = new Page();
    ...
    page.setProperties(notePageProperties);
    Wiki spaceWiki = getOrCreateWiki(wService, PortalConfig.GROUP_TYPE, space.getGroupId());
    Page note = noteService.createNote(spaceWiki, "Home", page, ROOT_IDENTITY);
    ...
    draftPage.setProperties(notePageProperties);
    draftPage = noteService.createDraftForExistPage(draftPage, note, null, new Date().getTime(), "root");
    featuredImage = noteService.getNoteFeaturedImageInfo(Long.valueOf(draftPage.getId()), null, true, null, getIdentityId(ROOT_IDENTITY.getUserId()));
    assertNotNull(featuredImage);
    noteService.removeDraftById(draftPage.getId());
  ...
  public void testGetDraftsOfWiki() throws Exception {
    Wiki portalWiki = getOrCreateWiki(wService, PortalConfig.PORTAL_TYPE, PORTAL_NAME);
    Page note = noteService.createNote(portalWiki, "Home", new Page("testGetDraftsOfWiki", "testGetDraftsOfWiki"), ROOT_IDENTITY);
    Page note2 = noteService.createNote(portalWiki, "Home", new Page("testGetDraftsOfWiki", "testGetDraftsOfWiki"), ROOT_IDENTITY);
    DraftPage draftPage = new DraftPage();
    draftPage.setTitle("test");
    draftPage.setContent("test");
    ...
    noteService.createDraftForExistPage(draftPage, note, null, new Date().getTime(), "root");
    assertEquals(2,
                 noteService.getDraftsOfWiki(portalWiki.getOwner(), portalWiki.getType(), portalWiki.getWikiHome().getName())
                            .size());
  }

  public void testSaveHideAuthorAndHideReactionProperties() throws Exception {
    Page note = createTestNoteWithVersionLang("testMetadataHideAuthorAndHideReaction", null, ROOT_IDENTITY);
    ...
    noteService.createVersionOfNote(note, ROOT_IDENTITY.getUserId());
    ...
    noteService.saveNoteMetadata(notePageProperties, "en", getIdentityId(ROOT_IDENTITY.getUserId()));

    // when fetch version language metadata properties we should return the
    // original note properties values in case of shared original properties values
    Page savedNote = noteService.getNoteByIdAndLang(Long.valueOf(note.getId()), "en");
    assertTrue(savedNote.getProperties().isHideAuthor());
    assertTrue(properties.isHideReaction());
  }
  ...
  public void testProcessingNoteContentImages() throws Exception {
    ...
    assertTrue(draftPage.getContent()
                        .contains(imageSrcTagSuffix.concat(WikiDraftPageAttachmentPlugin.OBJECT_TYPE)
                                                   .concat("/")
                                                   .concat(draftPage.getId())));
    //
    Page note = new Page();
    ...
    Wiki portalWiki = getOrCreateWiki(wService, PortalConfig.PORTAL_TYPE, PORTAL_NAME);
    note = noteService.createNote(portalWiki, "Home", note, ROOT_IDENTITY);

    // processing note version content images
    assertNotNull(note);
    assertNotNull(note.getId());
    assertTrue(note.getContent()
                   .contains(imageSrcTagSuffix.concat(WikiPageAttachmentPlugin.OBJECT_TYPE).concat("/").concat(note.getId())));
    ...
    draftForExistingPage = noteService.createDraftForExistPage(draftForExistingPage,
                                                               note,
                                                               null,
    ...
    assertTrue(draftForExistingPage.getContent()
                                   .contains(imageSrcTagSuffix.concat(WikiPageAttachmentPlugin.OBJECT_TYPE)
                                                              .concat("/".concat(note.getId()))));
  }
  ...
  public void testProcessingNoteImportedContentImages() throws Exception {
    ...
    content = content.substring(content.indexOf(imageSrcTagSuffix.concat(WikiPageAttachmentPlugin.OBJECT_TYPE)
                                                                 .concat("/")
                                                                 .concat(note.getId())));
