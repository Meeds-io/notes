...
public class WikiSpaceActivityPublisher extends PageWikiListener {
   ...
   * @param args
   */
  private void createAndSaveSystemComment(ExoSocialActivity activity, String userId, String commentMsgKey, Locale locale, String... args) {
    createAndSaveComment(activity, CommentType.SYSTEM, userId, null, commentMsgKey, args, null, null, locale);
  }
   ...
   * @param comment
   */
  private void createAndSaveUserComment(ExoSocialActivity activity, String userId, String comment, Locale locale) {
    createAndSaveComment(activity, CommentType.USER, userId, comment, null, null, null, null, locale);
  }
  ...
  private void createAndSaveComment(ExoSocialActivity activity,
    ...
    //
    if (userComment == null) {
      userComment = getValueFromResourceBundle(commentMsgKey1, args1, locale);
    }
    ...
    case SYSTEM_GROUP:
      ...
      if (args2 != null) {
        builder.append("<br/>").append(getValueFromResourceBundle(commentMsgKey2, args2, locale));
      } else {
   ...
   * @return
   */
  private String getValueFromResourceBundle(String key, String[] params, Locale locale) {
    ResourceBundleService resourceBundleService = CommonsUtils.getService(ResourceBundleService.class);
    ResourceBundle res = resourceBundleService.getResourceBundle("locale.wiki.integration.integration", locale);
